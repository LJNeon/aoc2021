...
fn input() -> [Line; LENGTH] {
  include_str!("../../inputs/day5.txt").lines().map(|n| Line::new(n)).into_array()
}
...
pub fn solve_a() -> usize {
  ...
  for (i, first) in lines.iter().enumerate() {
    for second in lines.iter().skip(i + 1) {
      if !first.is_straight() || !second.is_straight() {
        continue;
      } else if first.can_layer(second) {
        if first.c() == second.c() {
